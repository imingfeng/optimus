...
export default Model.extend({
  ...
  effects: {
    ...
    * fetchList({ payload }: any, { callWithSpinning, select, update }: any) {
    ...
    // 导出列表
    * fetchExportList({ payload }: any, { update, call, put, select }: any): any {
      const {
        filterParams,
        countStatus,
        list,
      ...
      if (list.length === 0) {
        message.error('筛选数据为空，不能导出');
        return
      }

      yield call(services.exportList, {
        status: countStatus,
        ...filterParams,
        ...payload,
      })
