...
import ChartBar from './../../components/barThree'
...
const Statistics: FC<StatisticsProps> = (props: any) => {
  ...
  let data1: any[] = [], data2: any[] = [], data3: any[] = [], xAxisDatas: any[] = [];
  barList.map((item: any) => {
    data1.push(item.incrNum)
    data2.push(item.stockNum)
    data3.push(item.noPhoneNum)
  ...
  return (
    <>
      <div className="contentPrimary">
        <h2 className="commonTit">新增蜂场统计（总计蜂场：{unitsSeparate(total)}）</h2>
